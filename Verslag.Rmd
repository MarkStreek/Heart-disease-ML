...
Vervolgens werd er gekeken naar de classificatie variabele. Deze variabele is belangrijk voor het ontwikkelen van het machine learning model. Het model wordt getraind op deze kolom in de data. De verdeling van dit attribuut wordt onderstaand weergegeven in tabel 1.
...
Zoals gezegd is het attribuut ST depression getransformeerd. Zonder normalisatie is er een hoge piek aanwezig bij de nullen. Met een log10 transformatie worden deze waardes allemaal -oneindig. Dit is niet goed bruikbaar en zal dus achterwege worden gelaten, de data is tenslotte nog steeds niet normaal verdeeld.
...
Bij een p-waarde lager dan 0.05 is er een significant verschil. Het attribuut Thallium heeft de laagste p-waarde en dus ook de meeste samenhang met de classificatie kolom. Verder zeggen de waardes Cholesterol en FBS over 120 weinig, want deze hebben een p-waarde van rond de 0.05 of groter dan 0.05.

Voordat er een machine learning model ontwikkeld kan worden, moeten er nog stappen worden uitgevoerd met de data. Om te beginnen is de eerste kolom verwijderd. Verder worden een aantal attributen omgezet in een label, dit om de dataset overzichterlijker en leesbaarder te maken. Als laatste worden er drie extra datasets aangemaak. Er worden datasets aangemaakt voor de volgende situaties: een dataset met de top 5 laagte p-waardes attributen, een dataset waarin alle numerieke kolommen zijn getransformeerd en een dataset waarin de 4 simpelste attributen aanwezig zijn. Deze datasets worden gebruikt voor het vergelijken van het model. Scoort een model even goed op een dataset met veel minder attributen? Dan kunnen wellicht overbodige attributen worden aangetoond.

# Discussie en Conclusie

## Discussie

In deze sectie zal er teruggekeken worden op de resultaten.

Allereerst de resultaten van de EDA. In deze sectie is er gekeken naar de eigenschappen van de data. Maar ook de samenhang tussen vele attributen. Om te beginnen zijn de buitenliggende waardes weergegeven. Dit werd gedaan door middel van een violin plot. In dit plot zijn naar verhouding, weinig buitenliggende waardes te zien. Er zijn weliswaar een aantal waardes die buiten de verdeling liggen, maar dit is niet significant.

De numerieke waardes bevatten geen lege waardes en zijn redelijk normaal verdeeld. De cholesterol waarde vertoond een (veel) betere normaalverdeling bij een log10 transformatie. Deze waarde zal dus verder worden gebruikt als transformatie. Dit samen met het feit dat de attributen weinig tot geen buitenliggende waardes bevat, hoeven de attributen verder niet te worden aangepast of gemodificeerd. 
...
Tot slot werd er in de EDA gekeken naar de p-waardes van statistische toetsen. Alle attributen in de dataset zijn getest met de ziek/niet ziek groep om de samenhang te bepalen. 11 van de 13 kolommen bevat een p-waarde lager dan 0.05. Dit betekend dat er 11 attributen zijn die een significante samenhang hebben met de ziek/niet ziet waarde. Al deze attributen zijn dus geschikt om een model op te bouwen.

## Conclusie

Voordat er begonnen werd aan het bouwen van een model is het belangrijk om de data te inspecteren. De dataset bevatte weinig opmerkelijk resultaten.

Allereerst waren er geen missende waardes aanwezig, éé attribuut is getransformeerd om de beste verdeling te hebben, er waren niet veel uitschieters aanwezig en meer dan dan 80% van de attributen had een grote samenhang met de classificatie kolom.
