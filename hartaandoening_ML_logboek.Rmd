...
The above graphs tells us that age is quite well distributed. BP and Cholesterol appear to be split more towards the left. And Max.HR more to the right.
...
```{r}
slices <- c(sum(data[15] == "Presence"), sum(data[15] == "Absence"))
lbs <- c("Presence", "Absence")
clr <- c("darkgreen", "violet")
pie(slices, labels = slices, col = clr)
legend("topright", c("Presence", "Absence"), fill = clr)

#pie(c(sum(data[15] == "Presence")), sum(data[15] == "Absence"), labels = c("X", "s"))
```
