...
ggplot(age.frame, aes(x = Patient, y = values, color=Patient)) + geom_boxplot() + labs(title = "Boxplot from patients age", subtitle = "Ages between presence and Absence") + theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 20), plot.subtitle = element_text(hjust = 0.5)) 
```
...
```{r}
test <- t.test(data$Age[data$Heart.Disease == "Presence"], data$Age[data$Heart.Disease == "Absence"], alternative = "greater")
...
related.frame <- data.frame(
  ...
  values = c(data$Cholesterol[data$Exercise.angina == "1"], data$Cholesterol[data$Exercise.angina == "0"]))
...
```{r}
test <- t.test(data$Cholesterol[data$Exercise.angina == "0"], data$Cholesterol[data$Exercise.angina == "1"], alternative = "two.sided")
cat("P-value: ", test$p.value)
```
...
related.frame <- data.frame(
  ...
  values = c(data$Max.HR[data$Heart.Disease == "Presence"], data$Max.HR[data$Heart.Disease == "Absence"]))

ggplot(related.frame, aes(x = labs, y = values, fill = labs)) + geom_boxplot()
```

```{r}
test <- t.test(data$Max.HR[data$Heart.Disease == "Presence"], data$Max.HR[data$Heart.Disease == "Absence"], alternative = "two.sided")
cat("P-value: ", test$p.value)
```
...
- BP and Age have the highest corr
...
In the last part of the EDA we will look at some similarities between ordinal values. First of all, let's see if age still has an influence. We do this in this section because it concerns a comparison with two ordinal groups. For this we must use the Chi-square test
