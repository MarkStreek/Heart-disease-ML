...
Now that the best model has been created with the best attributes, we can classify it in the weka explorer.

\newpage

## ROC curve of model

After you've run Logistic on set 4 in the Weka explorer, you can right-click on the result (in result list). Press the option: "visualize treshold curve" and then "absence". You can now store the data to make a ROC curve in R. Press save and enter a destination.
...
ggplot(ROC.data, aes(x = False_Positive_Rate, y = True_Positive_Rate)) + 
  geom_line(color = "orange", size = 1.2) + labs(title = "ROC curve from Logistic", subtitle = "Logistic was run on Set 4 (See above)") + theme(legend.position = "bottom", plot.title = element_text(face = "bold", 
