...
The F-score indicates the accuracy of a concept. Let's look at the
F-scores.
...
In the Weka explorer, under functions there is a classifier "CostSensitiveClassifier". After selecting this classifier, open the options and resize the scoring matrix. After resizing, select a score of 4 for the false positives. Select Logistic as classifier and press start.
...
There are 11 instances that are classified as Absence but are Presence in fact.

Let's run Logistic again, but then with default cost. The folowing scoring matrix appears
...
Stacking with Logistic as meta classifier achieves an accuracy of 86.6667%, which again is higher. Stacking with NaiveBayes achieves an accuracy of 87.037%! The False Positve rate is also lower, 0.16 vs 0.19. Fals negative rate is the same.
...
This method achieves an accuracy of 85.1852% with Logistic as classifer and 83.7037% with NaiveBayes.
...
The curve has a high deflection to the upper left, which is good for a roc curve. The area under the curve is 0.91.
