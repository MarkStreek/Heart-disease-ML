...
As described in the introduction, the attributes in the data set must first be examined more closely. This is done using a code book. This codebook is then also used to put the attributes in the correct data type.
...
c1 <- ggplot(data, aes(x=Age)) + geom_histogram(bins = 15) + 
  labs(title = "Distribution Age Attribute")
...
The data of ST depression and cholesterol is log10 transformed. This is done because the data showed quite a different view. With an log10 transformation, the data is more observable.
...
age.frame <- data.frame(
  ...
  values = c(data$Age[data$Heart.Disease == "Presence"], 
             data$Age[data$Heart.Disease == "Absence"])
)

ggplot(age.frame, aes(x = Patient, y = values, color=Patient)) + 
...
related.frame <- data.frame(
  ...
  values = c(data$Cholesterol[data$Exercise.angina == "1"], 
...
As told earlier, some columns have been log10 transformed. We are now going to make a 'clean' dataset. I.e., a dataset with usable columns and ...

resultaten zijn objectief en de discussie is subjectief.
