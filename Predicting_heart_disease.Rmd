...
Naive bayes scores highest accuracy. In the tests below, this will be put first to detect concepts that are significantly better than Naive bayes. Let's look at other comparison fields.
...
K-nearest neighbours and SMO show the highest Area_under_ROC after Naive Bayes. 
...
For the false negative rate there are two values that are significantly lower than for NaiveBayes. However, these values are in the "simple" dataset. This dataset does not score highly (this is shown below).

For the flase positive rate only zeroR scores lower. But that makes sense.
...
Now all that remains is to look for the best dataset.
